/**
 * M3 e2e (no UI): Orchestrator end-to-end with real Codex provider in stateful_resume mode.
 *
 * Validates:
 * - thread_id extraction (providerSessionId) from JSONL thread.started
 * - resume path is actually used for turns >= 2 (manager + executor)
...
async function main() {
  ...
  const managerSession = store.createSession({
    ...
    configJson: JSON.stringify({
      ...
      systemPromptPath: 'prompts/tests/m3_e2e_codex_resume_manager_system.md',
    }),
  });
  ...
  console.log(`[m3_e2e_codex_resume] PASS: run=${finalRun.id} turns=${turns.length} outDir=${outDir}`);
  store.close();
}

main().catch((err) => {
  console.error(`[m3_e2e_codex_resume] FAIL: ${err.message}`);
  process.exit(1);
});
